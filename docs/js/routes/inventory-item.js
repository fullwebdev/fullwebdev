      ...
      <table>
        ${data.bundle.gzip
          ? html`<tr>
              ...
              <td>${data.bundle.gzip}</td>
            </tr>`
          : ""}
        ${data.bundle.dlTime
          ? html`<tr>
              <td>Download Time (regular 2G)</td>
              <td>${data.bundle.dlTime}</td>
            </tr>`
          : ""}
        ${data.showDependencies
          ? html`
              <tr>
                <td>Dependencies</td>
                <td>
                  ${data.bundle.dependencies.names.length > 0
                    ? html` ${data.bundle.dependencies.names.join(", ")} `
                    : "none"}
                </td>
              </tr>
              ${data.bundle.dependencies.size
                ? html`<tr>
                    <td>Proportion of total size taken up by dependencies</td>
                    <td>${data.bundle.dependencies.proportion}</td>
                  </tr>`
                : ""}
            `
          : ""}
...
export default ({ data, routeParams: [, pkgName] }) => {
  const name = decodeURIComponent(pkgName);
  let content;
  if (!data) {
    content = html`<div class="loading">
      Loading...
    </div>`;
  } else if (data.error) {
    content = html`<div class="error">${data.error}</div>`;
  } else if (data.npmInfos) {
    content = html`
      ...
      ${bundleInfos(data)} ${npmScore(data.npmInfos.score)}
      ${npmLinks(data.npmInfos.links)}
    `;
  } else {
    content = "no npm package found";
  }

  return html`
    <p><a href="/en/03-inventory/">&lt; back to inventory</a></p>
...
export async function fetchData([, pkgName]) {
  const name = decodeURIComponent(pkgName);
  ...
  if (data.isALibrary) {
    try {
      ...
      /**
       * @type {{size: number | string, names: string[], proportion?: number | string }}
       */
      let dependencies = {
        size: 0,
        names: [],
      ...
      if (sizeData.gzip) {
        ...
        const dlTime = Math.round((sizeData.gzip / (30 * 1024)) * 1000);
        data.bundle = {
          gzip: toSizeStr(sizeData.gzip),
          dlTime:
            dlTime > 999 ? `${Math.round(dlTime * 100) / 100}s` : `${dlTime}ms`,
          dependencies,
        };
      }
    } catch (err) {
